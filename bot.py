...
# =============================================================================
# ДОСТУП
# ADMIN_IDS — полный доступ: бот + команда /admin с панелью управления.
...
USER_IDS: set[int] = {
    # сюда добавляй ID пользователей которым разрешён доступ к боту
    # например: 123456789,
}
...
def is_allowed(user_id: int) -> bool:
    """Доступ к боту есть у админов и пользователей из USER_IDS."""
...
# Лог событий: каждая запись — dict с полями time, user_id, username, action
# Хранится в памяти, максимум 500 последних событий
event_log: list[dict] = []
MAX_LOG = 500

def log_event(user: types.User, action: str) -> None:
    """Записывает действие пользователя в лог событий."""
    event_log.append({
...
def get_user(uid: int, tg_user: types.User | None = None) -> dict:
    ...
    if tg_user is not None:
        ...
        user_data[uid]["name"]     = f"{tg_user.first_name or ''} {tg_user.last_name or ''}".strip() or "—"
...
def handle_admin(message: types.Message):
    try:
        ...
        users_today = len({
            e["user_id"] for e in event_log
            if e["time"].date() == today_msk
        })

        # Последние 5 событий
        last_events = event_log[-5:][::-1]
        events_text = "\n".join(
            f"  `{e['time'].strftime('%H:%M')}` @{e['username']} — {e['action']}"
            for e in last_events
        ) or "  нет событий"

        # Список всех пользователей бота
        users_text = "\n".join(
            f"  {'👑' if uid in ADMIN_IDS else '👤'} `{uid}` @{d.get('username','—')} — "
            f"билетов: {d.get('tickets_count', 0)}"
        ...
        log.info("Админ-панель открыта: user_id=%s", message.from_user.id)
    except Exception as e:

...
# Хранилище PDF-билетов: token → (pdf_bytes, expires_at)
# Тот же TTL что у HTML (1 час). Ключ совпадает с ticket_store —
# один token даёт доступ и к HTML, и к PDF.
pdf_store: dict[str, tuple] = {}
...
@flask_app.route("/pdf/<token>")
def serve_pdf(token: str):
    """Отдаёт PDF-билет по уникальному токену."""
    ...
    return pdf_bytes, 200, {
        ...
        "Content-Disposition": f'attachment; filename="ticket_{token[:8]}.pdf"',
    }
...
def build_html(route: str, vehicle: str, payment_unix: int,
               ticket_serial: str | None = None,
               ticket_number: str | None = None) -> bytes:
    ...
    html = html.replace("{{TICKET_SERIAL}}", ticket_serial or generate_ticket_serial())
    html = html.replace("{{TICKET_NUMBER}}", ticket_number or generate_ticket_number(payment_dt))
...
# ГЕНЕРАЦИЯ PDF
#
# Структура повторяет оригинал (см. образец):
#   • Дата/время вверху
#   • Таблица реквизитов (метка серым, значение чёрным жирным)
#   • QR-код по центру внизу
#
# Зависимости: reportlab, qrcode[pil], Pillow
# Кириллица: встраиваем шрифт DejaVuSans из пакета dejavu-fonts-ttf или
#             скачиваем при первом старте с GitHub (fonts/).
# =============================================================================

def _get_font_path() -> str:
    """
    Возвращает путь к файлу DejaVuSans.ttf.
    Сначала ищет рядом со скриптом (fonts/DejaVuSans.ttf),
    затем пробует скачать с GitHub dejavu-fonts.
    """
    ...
    log.info("Скачиваю DejaVuSans.ttf из %s", url)
    urllib.request.urlretrieve(url, path)
    log.info("Шрифт сохранён: %s", path)
    return path


def _ensure_fonts() -> None:
    """Регистрирует шрифт DejaVuSans в reportlab (только один раз)."""
    from reportlab.pdfbase import pdfmetrics
    from reportlab.pdfbase.ttfonts import TTFont
    if "DejaVuSans" not in pdfmetrics.getRegisteredFontNames():
        pdfmetrics.registerFont(TTFont("DejaVuSans",      _get_font_path()))
        pdfmetrics.registerFont(TTFont("DejaVuSans-Bold", _get_font_path()))
        log.info("Шрифт DejaVuSans зарегистрирован в reportlab")
...
def build_pdf(
    ...
    """
    Генерирует PDF-билет в стиле оригинала (qr.bilet.nspk.ru → «Скачать билет»).

    Поля:
      Вид билета          — Разовый билет (QR) в Мурманске
      Серия билета        — ticket_serial
      Номер билета        — ticket_number
      ИНН перевозчика     — 5190193597
      Наименование        — АО Электротранспорт города Мурманск
      Вид транспорта      — Автобус
      Маршрут/Станция     — route
      Номер ТС            — vehicle
      Дата и время        — payment_unix → дд.мм.гггг чч:мм
      Стоимость           — 53.0 руб.

    QR-код кодирует номер билета (как в оригинале).
    """
    import qrcode
    ...
    payment_dt  = datetime.fromtimestamp(payment_unix, tz=MSK)
    dt_str      = payment_dt.strftime("%d.%m.%Y %H:%M")
    page_w, page_h = A6   # 105 × 148 мм

    # ── QR-код ────────────────────────────────────────────────────────────────
    # Оригинал кодирует номер билета в QR
    qr = qrcode.QRCode(
        version            = None,
    ...
    margin_l = 8 * mm
    margin_r = page_w - 8 * mm
    y        = page_h - 10 * mm   # текущая позиция сверху вниз

    # Дата/время вверху (как в оригинале)
    c.setFont("DejaVuSans", 9)
    c.setFillColor(colors.black)
    c.drawString(margin_l, y, dt_str)
    y -= 7 * mm
    ...
    # Реквизиты: список (метка, значение)
    fields = [
        ("Вид билета",              "Разовый билет (QR) в Мурманске"),
        ("Серия билета",            ticket_serial),
        ("Номер билета",            ticket_number),
        ("ИНН перевозчика",         "5190193597"),
        ("Наименование перевозчика","АО Электротранспорт города Мурманск"),
        ("Вид транспорта",          "Автобус"),
        ("Маршрут/Станция",         route),
        ("Номер ТС",                vehicle),
        ("Дата и время поездки",    dt_str),
        ("Стоимость",               "53.0 руб."),
    ]

    label_size = 6.5
    value_size = 9.5
    row_gap    = 1.5 * mm   # отступ между строками одной записи
    field_gap  = 3.5 * mm   # отступ между записями

    for label, value in fields:
        # Метка — мелкий серый текст
        c.setFont("DejaVuSans", label_size)
        c.setFillColor(colors.HexColor("#888888"))
        c.drawString(margin_l, y, label)
        y -= row_gap + label_size * 0.352778 * mm  # points → mm

        # Значение — крупный чёрный текст
        c.setFont("DejaVuSans", value_size)
        c.setFillColor(colors.black)
        # Длинный текст (перевозчик) переносим вручную
        if len(value) > 32:
            words  = value.split()
            line1  = ""
            line2  = ""
            ...
            y -= value_size * 0.352778 * mm
            if line2:
                c.drawString(margin_l, y, line2)
                y -= value_size * 0.352778 * mm
        else:
            c.drawString(margin_l, y, value)
            y -= value_size * 0.352778 * mm

        y -= field_gap

    # Небольшой отступ перед QR
    y -= 3 * mm
...
def ticket_keyboard(token: str, route: str, has_pdf: bool = True) -> types.InlineKeyboardMarkup:
...
def _cleanup_expired_tickets() -> int:
    ...
    for t in expired:
        ...
        pdf_store.pop(t, None)   # PDF удаляем вместе с HTML по тому же токену
    return len(expired)
...
def _send_ticket(
    ...
    # Генерируем PDF с теми же реквизитами что и HTML.
    # serial и number берём из build_html — генерируем заново со своими seed,
    # поэтому передаём их явно чтобы HTML и PDF совпадали.
    # Для совпадения — передаём одни и те же serial/number в обе функции.
    # Решение: генерируем serial/number один раз здесь и передаём в обе.
    t_serial = generate_ticket_serial()
    t_number = generate_ticket_number(datetime.fromtimestamp(payment_unix, tz=MSK))
    # Пересобираем html_bytes с фиксированными serial/number
    try:
        html_bytes = build_html(route, vehicle, payment_unix, t_serial, t_number)
        ticket_store[token] = (html_bytes, expires_at)
    except Exception:
        pass  # если что-то пошло не так — старый html уже лежит в store

    try:
        pdf_bytes = build_pdf(route, vehicle, payment_unix, t_serial, t_number)
        pdf_store[token] = (pdf_bytes, expires_at)
    except Exception as e:
        log.warning("Не удалось сгенерировать PDF: %s", e)
        pdf_bytes = None
    ...
    bot.send_message(
        ...
        parse_mode="Markdown",
        reply_markup=ticket_keyboard(token, route, has_pdf=(pdf_bytes is not None)),
    )

...
def _load_user_data() -> None:
    ...
    """
    if not _gh_available():
        log.warning("GitHub не настроен — user_data НЕ будет персистентным!")
        return
...
def _save_user_data_async() -> None:
    """
    Асинхронно сохраняет user_data в user_data.json на GitHub.
    Запускается в daemon-потоке чтобы не блокировать ответ бота.
    _save_lock исключает гонку если два запроса придут одновременно.
    """
    def _worker():
        with _save_lock:
    ...
    threading.Thread(target=_worker, daemon=True, name="save-userdata").start()
